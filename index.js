...
function booleanJSONCNF(expression) {
  // Since boolean-json-schama@3.0.0, conjunctions and disjunctions can contain
  // more than two operands. This makes entering data more user-friendly.
  //
  // For example, `{ and: [ 'p', 'q', 'r' ] }` is now valid Boolean JSON, where
  // that would have to be expressed `{ and: [ 'p', { and: [ 'q', 'r' ] } ] }`
  // prior to 3.0.0.
  //
  // To end up with unambiguous CNF, first "bifurcate" the expression, which is
  // to say convert all conjunctions and disjunctions with more than two
  // operands to compound statements of two operands each.
  //
